...
export const themeService = {
    ...
    getCurrentTheme: () => localStorage.getItem(THEME_STORAGE_KEY) || "dark",
    setBackgroundImage: (url: string) => {
        presetSettings.backgroundImage = url;
        saveCardSettings();
        applyPreset();
    },
    setBackgroundBlur: (blur: number) => {
        presetSettings.backgroundBlur = blur;
        saveCardSettings();
        applyPreset();
    },
    setBackgroundOpacity: (opacity: number) => {
        presetSettings.backgroundOpacity = opacity;
        saveCardSettings();
        applyPreset();
    },
    getBackgroundImage: () => presetSettings.backgroundImage || "",
    getBackgroundBlur: () => presetSettings.backgroundBlur || 0,
    getBackgroundOpacity: () => presetSettings.backgroundOpacity || 100,
};
