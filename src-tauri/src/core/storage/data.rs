...
impl Data {
    ...
    pub async fn download_from_url(&self, url: &str, dest_path: &str) -> Result<(), String> {
        let root_dir = self.root_dir_snapshot();
        let full_dest_path = root_dir.join(dest_path);
        ...
        if let Some(app_handle) = APP_HANDLE.lock().unwrap().as_ref() {
            emit_to_main_window(app_handle, "download-start", dest_path);
        }
        ...
        if let Some(handle) = app_handle.as_ref() {
            emit_to_main_window(handle, "download-complete", dest_path);
        }
