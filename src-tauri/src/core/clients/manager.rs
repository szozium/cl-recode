...
impl ClientManager {
    ...
    pub async fn fetch_clients() -> Result<Vec<Client>, DynError> {
        ...
        // Filter out clients below 1.16.5
        clients.retain(|client| {
            let version = &client.version;
            if let Some((major, rest)) = version.split_once('.') {
                if let (Ok(maj), Some((minor, patch))) = (major.parse::<u32>(), rest.split_once('.')) {
                    if let (Ok(min), Some(pat)) = (minor.parse::<u32>(), patch.split('.').next().and_then(|p| p.parse::<u32>().ok())) {
                        return maj > 1 || (maj == 1 && (min > 16 || (min == 16 && pat >= 5)));
                    }
                }
            }
            false
        ...
        // Filter allowed clients only
        let allowed_clients = [
            "Venus", "NewCode", "ZenithRecode", "Haruka", "NightDLC", "Dimasik",
            "Catlean", "Astera", "Wissend", "LiquidBounceFabric", "Wurst",
            "KimaDLC", "KomaruDLC", "Excellent", "Excellent Upgrade", "Excellent Recode",
